...
export default {
  ...
  mounted() {
    ...
    window.addEventListener('message', (event) => {
      console.log("触发了2：",event);
      var data = event.data;
      if(data.code=='initMind'){
        var mindData = data.mind;
        that.setData(mindData);
      }
    });
  ...
  methods: {
    ...
    async saveLocalFile() {
      ...
      // console.log(window.parent.homePageCallBack);
      // window.parent.homePageCallBack();
      var result = {
        code:'saveMind',
        mind:data
      }
